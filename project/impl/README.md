...
```shell
: ${LAB_DOMAIN:=.fcc21.fogx.me}  # Our lab domain
: ${IAAS_PREFIX:=iaas-}  # Our lab prefix
: ${PAAS_PREFIX:=paas-}  # Our lab prefix

: ${EMAIL:=matteo.franzil@studenti.unitn.it}  # Your unitn email as provided
...
: ${DOCKER_REG_USER:=fcc21}
: ${DOCKER_REG_PASSWORD:=FcC21Password123}
...
: ${SQL_USER:=sqldiciotto}
: ${SQL_PASSWORD:=2bhturifjnbgtru8ei2938euj}
: ${SQL_DB:=postgres}
...
```shell
openstack user create --project fcc21 --password ${EVAL_ACC} ${EVAL_ACC}
openstack role add --user ${EVAL_ACC} --project fcc21 reader
```
